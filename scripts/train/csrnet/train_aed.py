...
def main():
    arg_parser = argparse.ArgumentParser(description="Train CSRNet on AED dataset")
    arg_parser.add_argument("--override_data_root", type=str, default=None, help="Override default data root")

    ROOT = Path(__file__).resolve().parents[3]
    OUTPUT_DIR = ROOT / "results" / "csrnet" / "aed"
    data_root = ROOT / "data" / "splits" / "aed"
    ...
    train_dataset = AEDDataset(root=data_root, split="train")
    val_dataset = AEDDataset(root=data_root, split="val")
